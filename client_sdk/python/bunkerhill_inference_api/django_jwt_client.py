...
from .exceptions import InferenceAPIRequestFailedError, JSONResponseParseError
...
class DjangoJWTClient:
    ...
    async def _parse_response_json(
        self, url: str, response: aiohttp.ClientResponse, action: str
    ) -> Any:
        if not response.ok:
    ...
    async def _authorize(self, session: aiohttp.ClientSession):
        ...
        client_jwt = jwt.encode(
            data_to_encode, self._client_private_key, algorithm=self.CLIENT_JWT_ENCODING_ALGORITHM
        )
        self._access_jwt = await self._send_authorization_request(client_jwt, session)

    @retry(tries=3, delay=1, backoff=2)
    async def _send_authorization_request(
        self, client_jwt: str, session: aiohttp.ClientSession
    ) -> str:
    ...
    def _should_reset_access_jwt(self) -> bool:
        return (
            self._is_jwt_expired_or_invalid()
            or self._num_failures % self._num_failures_allowed == 1
        )
